  ...
  if (todoArr) {
    for (let i = 0; i < todoArr.length; i++) {
      if (todoArr[i].id === id) {
        ...
        td1.innerHTML = todoArr[i].title; td2.innerHTML = todoArr[i].description;
        td3.innerHTML = todoArr[i].dueDate; td4.innerHTML = todoArr[i].priority;
...
addProject.addEventListener('click', () => {
  ...
  const createProject = document.createElement('input');
  ...
  const submitProject = document.querySelector('.new_project');
  submitProject.addEventListener('click', () => {
    const latestProject = document.querySelector('.new_project_value').value;
    if (latestProject !== '') {
...
const addTodos = document.querySelector('.add_todos');
